...
@export var piece_length: float = 2.0
@export var pieces_ahead: int = 8
...
func _process(_delta: float) -> void:
	...
	if active_pieces.size() > 0:
		...
		if ball.global_position.z - last_piece_z < piece_length * 3.0:
			_spawn_piece()

	while active_pieces.size() > 0 and active_pieces[0].global_position.z - ball.global_position.z > piece_length * 3.0:
